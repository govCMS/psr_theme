      ...
      if ($slide.size() > 1) {
        ...
        if (settings.pager) {
          ...
          $slide.each(function (i) {
            var n = i + 1;
            tabMarkup +=
              '<button><span class="element-invisible">Slide</span> ' +
              '<span class="pager-text' + slideClassPrefix + n + '">' + n + '</a>' +
              '</button>';
          });
          ...
          selectTab = function (idx) {
            $tabs.closest('button').find('span.selected').remove();
            $tabs
              .closest('button')
              .removeClass(activeClass)
              .eq(idx)
              .addClass(activeClass)
              .append($('<span class="selected element-invisible">- Selected</span>'));
          };
        }
        ...
        if (settings.pauseButton) {
            var pause_link = '<button class="pause playing"><img src="' + settings.pauseImgUrl + '" alt="Pause" /></button>';

            $('.rslides_tabs').append(pause_link);
